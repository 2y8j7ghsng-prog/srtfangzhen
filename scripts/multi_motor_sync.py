...
多电机协作（同步）控制仿真
========================================
三台 PMSM 简化模型（转速环 PI + 转矩饱和），对比三种经典同步策略：
  1. 主从控制（Master-Slave）
...
def load_torque(t):
    """三台电机的负载转矩 (N·m)"""
    tl = np.zeros(N_MOTOR)        # 先整体清零：1 号机全程空载
    if t >= T_LOAD_STEP:
...
def run_master_slave():
    """主从控制：电机 1 为主机，2/3 号直接跟踪主机实际转速"""
    w = np.zeros((STEPS, N_MOTOR))       # 预存整条转速轨迹 [步, 电机]
    integ = np.zeros(N_MOTOR)            # 三路转速环 PI 积分器
    for k, t in enumerate(T):
        w_ref = speed_ref(t)
        refs = np.array([w_ref, w[k - 1, 0] if k > 0 else 0.0,
                         w[k - 1, 0] if k > 0 else 0.0])
        # refs[0] 主机跟踪给定；refs[1..2] 从机跟踪主机"上一拍"转速
...
def run_cross_coupling():
    ...
    integ_track = np.zeros(N_MOTOR)      # 三路转速环 PI 积分器
    integ_sync = np.zeros(N_MOTOR)       # 三路同步补偿积分器 ∫ε_i dt
    for k, t in enumerate(T):
        w_ref = speed_ref(t)
        tl = load_torque(t)
        for i in range(N_MOTOR):
...
def main():
    ...
    # 3 行 1 列；sharex/sharey 让三格坐标范围一致，便于纵向比较
